...
<article class="obj_article_details">
  ...
  <div class="row">
    <div class="main_entry">
            ...
            {foreach from=$publication->getData('authors') item=author}
              <li>
                ...
                {if $author->getLocalizedData('affiliation')}
                  <span class="affiliation">{$author->getLocalizedData('affiliation')|escape}
      ...
      {/if}
  {* --- DOI + Crossmark Integration START --- *}
{* --- DOI + Crossmark Integration START --- *}
{foreach from=$pubIdPlugins item=pubIdPlugin}
  ...
  {if $pubId}
    ...
    <section class="item doi">
      ...
      {* Crossmark logo button right below DOI, no label *}
      <div style="margin-top: 0.5rem;">
        <a href="{$doiUrl}" target="_blank" onclick="document.getElementById('crossmark-modal').style.display='block'; return false;">
              <img src="{$baseUrl}/public/site/CROSSMARK_Color_horizontal.svg" alt="Crossmark Logo" style="width:150px; height:auto; display:inline-block; margin:0 auto;" />
        </a>
      </div>
    </section>

    {* Crossmark modal (outside the section for cleaner structure) *}
    <div id="crossmark-modal" style="display:none; position:fixed; top:5%; left:5%; width:90%; height:90%; background:#fff; z-index:9999; border:2px solid #ccc;">
      <div onclick="document.getElementById('crossmark-modal').style.display='none'" 
           style="position:absolute; top:5px; right:10px; cursor:pointer; font-weight:bold; z-index:10000;">✖
      </div>
      <iframe src="https://crossmark.crossref.org/dialog/?doi={$pubId|escape:'url'}&domain=pdf" width="100%" height="100%" frameborder="0"></iframe>
    </div>

  {/if}
{/foreach}
{* --- DOI + Crossmark Integration END --- *}
  {* --- DOI + Crossmark Integration END --- *}
    ...
    <div class="entry_details">
      ...
      {* PubIds (requires plugins) except DOI (already shown above) *}
      {foreach from=$pubIdPlugins item=pubIdPlugin}
        {if $pubIdPlugin->getPubIdType() == 'doi'}
          {continue}
        {/if}
